...
<script type="text/javascript">
	function getDataAC(acType, acTypeName) {
		...
		$.ajax({
			url: '<?php echo site_url() . '/health_view/get_ac_reg';?>',
			type: 'post',
			dataType: 'json',
			...
			if(result.status == 'ok') {
				var resultData = $('[name="searchResult"]'),
					myBaseUrl = '<?php echo site_url();?>/health_view/cabin/'+acTypeName;
				//clear acType
				...
				$.each(result.result, function(index, value) {
					var row = template.clone().removeClass('hidden').removeAttr('template'),
						new_value = (parseFloat(value.performance_interior) + parseFloat(value.performance_exterior)) / 2;
