...
class Page {
	...
	private $_mainmenu		= 'templates/menu_tpl';		// page header
	...
	public function __construct() {
		...
		if ($this->_CI->uri->segment(2) == 'pdf' OR $this->_CI->uri->segment(3) == 'pdf') {
			...
			elseif($this->_CI->session->userdata('users_logged_in') != '') {
				redirect(site_url('/dashboard/'));
			}
		}
		else {
			if($this->_CI->session->userdata('users_logged_in') == '') {
				redirect(base_url());
			}
			else{
	...
	function mainmenu($mainmenu) {
		return $this->_mainmenu = $mainmenu;
	}
